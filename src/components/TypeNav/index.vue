...
export default {
  ...
  mounted() {
    // 通过store对象的dispatch()来触发异步action getCategorysList执行请求获取数据
    // this.$store.dispatch('getCategorysList')  // 数据从接口中转移到vuex的state中
    // this.getCategorysList();
  },

  methods: {
    // getCategorysList () {this.$store.dispatch('getCategorysList')}
